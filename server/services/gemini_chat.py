...
Two auth surfaces are supported so the key from render.env works either way:
  * `AIza...` API key            → Google AI Studio (generativelanguage.googleapis.com)
  * OAuth/GCP token (`AQ.`/`ya29.`) → Vertex AI on this project's region (Bearer)
...
from ..config import CACHE_DIR, EE_PROJECT

GEMINI_KEY = os.environ.get("GEMINI_API_KEY", "")
GEMINI_MODEL = os.environ.get("GEMINI_MODEL", "gemini-2.5-flash")
GEMINI_PROJECT = os.environ.get("GEMINI_PROJECT", EE_PROJECT)
...
def _endpoint() -> tuple[str, dict]:
    ...
    return (
        f"https://{GEMINI_REGION}-aiplatform.googleapis.com/v1/projects/{GEMINI_PROJECT}"
        f"/locations/{GEMINI_REGION}/publishers/google/models/{GEMINI_MODEL}:generateContent",
